...
(def schemeUpdate
  {:meal_id       [st/required st/number-str]
   ...
   :meal_type     [st/required st/number-str st/positive]
   })
...
(defn delete [id]
  (jdbc/db-do-commands mysql-db (jdbc/execute! mysql-db
                                               ["DELETE FROM meal WHERE meal_id = ?" id])))

(defn update [id params]
  (if (st/valid? params schemeUpdate) (jdbc/db-do-commands mysql-db (jdbc/update! mysql-db :meal params (sql/where {:meal_id id})))
                                      (throw (Exception. "Validation failed"))))

(defn insertMeal [params]
  (if (st/valid? params schemeInsert) (jdbc/db-do-commands mysql-db (jdbc/insert! mysql-db :meal params))
                                      (throw (Exception. "Validation failed"))))
